...
public class PreferencesPanel extends JPanel {
	...
	private void init(Properties props) {
		...
		inkscapeButton
				.setToolTipText("The location of the Inkscape executable on your system");
		inkscapeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (inkscapeChooser.showOpenDialog(SettingsDialog.parent) == JFileChooser.APPROVE_OPTION) {
					setInkscapePath(inkscapeChooser.getSelectedFile()
							.getAbsolutePath());
				}
			}
		});
		...
		constraints = new GridBagConstraints(1, 0, 1, 1, 1, 0,
				GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 0, 0);
		add(inkscapeField = new JTextField(20), constraints);
		inkscapeField.setText(props
				.getProperty(ConversionSVG.KEY_INKSCAPE_PATH) != null ? props
				.getProperty(ConversionSVG.KEY_INKSCAPE_PATH) : null);

		// Core and Maximum Threads in ThreadPool
		constraints = new GridBagConstraints(0, 1, 1, 1, 1, 0,
				GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 0, 0);
		add(SettingsDialog.createLabeledField("Core Threads",
				"The number of threads guaranteed to be in the thread pool"),
				constraints);
		constraints = new GridBagConstraints(0, 2, 1, 1, 1, 0,
				GridBagConstraints.NORTHWEST, GridBagConstraints.HORIZONTAL,
				new Insets(5, 5, 5, 5), 0, 0);
		add(SettingsDialog.createLabeledField("Max Threads",
	...
	public void setInkscapePath(String path) {
		props.setProperty(ConversionSVG.KEY_INKSCAPE_PATH, path);
	}
}
